...
def get_pets(db: Session, include_inactive: bool = False, current_user_id: int = None) -> List[Pet]:
    ...
    for pet in all_pets:
        ...
        if is_owner:
            ...
            if has_items:
                visible.append(pet)
        # Private entities are invisible to non-owners
...
def get_daily_summary(db: Session, care_day: date = None, current_user_id: int = None, kiosk_mode: bool = False) -> dict:
    """
    ...
    if kiosk_mode:
        pets = [p for p in pets if p.name.lower() == "chessie"]
