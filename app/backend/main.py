...
def check_timers_job():
    ...
    Privacy-aware: private pets only send alerts to their owner, not all users.
    """
    db = SessionLocal()
    try:
        now = datetime.now()
        ...
        if expired_pets:
            ...
            if alert_users:
                for pet in expired_pets:
                    ...
                    # Private pets: only notify the owner
                    if pet.is_private and pet.owner_id:
                        recipients = [u for u in alert_users if u.id == pet.owner_id]
                    else:
...
def nightly_reminder_job():
    ...
    Privacy-aware: each user only sees private items/pets they own in their summary.
    ...
    try:
        ...
        for user in users:
            ...
            for pet_data in summary:
                ...
                # Skip entire pet if it's private and this user isn't the owner
                if pet.is_private and pet.owner_id and pet.owner_id != user.id:
                    continue
                ...
                for t in pet_data['tasks']:
                    if t['is_completed']:
                        continue
                    item = t['care_item']
                    # Skip individual private items that belong to another owner
                    if item.is_private and item.owner_id and item.owner_id != user.id:
                        continue
