...
class TestPrivateItemAlerts:
    """Tests that non-public pets/items only send text alerts to their creator."""
    ...
    def test_private_pet_timer_alert_only_sent_to_owner(
        ...
        """A non-public pet's timer alert should only go to the creator, not all users."""
        ...
        # Non-public pet created by 'Owner'
        pet = Pet(name="Secret", species="cat", is_public=False, created_by=owner.id)
        db_session.add(pet)
        db_session.commit()
    ...
    def test_public_pet_timer_alert_sent_to_everyone(
        ...
        """A public pet's timer alert should go to all opted-in users."""
        today = date(2026, 3, 28)
        mock_get_care_day.return_value = today
        ...
        pet = Pet(name="Buddy", species="dog")  # is_public defaults to True
        db_session.add(pet)
        db_session.commit()
    ...
    def test_nightly_reminder_excludes_private_pet_from_non_owner(
        ...
        """Nightly reminder should not include non-public pet tasks for non-creators."""
        today = date(2026, 3, 28)
        mock_get_care_day.return_value = today
        ...
        # Non-public pet with an incomplete care item
        private_pet = Pet(name="Secret", species="cat", is_public=False, created_by=owner.id)
    ...
    def test_nightly_reminder_excludes_private_care_item_from_non_owner(
        ...
        """A non-public care item on a public pet is excluded from non-creators' reminders."""
        today = date(2026, 3, 28)
        mock_get_care_day.return_value = today
        ...
        # One public item, one non-public item
        ...
        private_item = CareItem(
            ...
            is_public=False, created_by=owner.id,
        )
